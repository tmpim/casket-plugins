...
func init() {
	casket.RegisterPlugin("realip", casket.Plugin{
		ServerType: "http",
		Action:     Setup,
	})
}
...
func parse(m *module, c *casket.Controller) (err error) {
...
func addIpRanges(m *module, c *casket.Controller, ranges []string) error {
...
//
// Helpers below here could potentially be methods on *casket.Controller for convenience
//

// IntArg check's there is only one arg, parses, and returns it
func IntArg(c *casket.Controller) (int, error) {
	args := c.RemainingArgs()
	if len(args) != 1 {
		return 0, c.ArgErr()
	}
...
func StringArg(c *casket.Controller) (string, error) {
	args := c.RemainingArgs()
	if len(args) != 1 {
		return "", c.ArgErr()
	}
...
func CidrArg(c *casket.Controller) (*net.IPNet, error) {
	a, err := StringArg(c)
	if err != nil {
		return nil, err
	}
...
func BoolArg(c *casket.Controller) (bool, error) {
	args := c.RemainingArgs()
	if len(args) > 1 {
...
func NoArgs(c *casket.Controller) error {
