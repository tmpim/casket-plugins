...
func (c *caddyTestServer) marshal() []byte {
	...
	if len(c.HTTPRedirectPort) > 0 {
		...
		// https://github.com/tmpim/casket/blob/master/caddyhttp/httpserver/https.go#L142 can change in future
		// and we won't know.
...
func (c *caddyTestServer) StartTestServer() {
	var err error
	c.Instance, err = casket.Start(casket.CaddyfileInput{Contents: c.marshal(), ServerTypeName: "http"})
	if err != nil {
		panic(err)
	}
	if c.contents == nil {
