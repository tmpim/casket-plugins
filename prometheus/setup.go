...
func init() {
	casket.RegisterPlugin("prometheus", casket.Plugin{
		ServerType: "http",
		Action:     setup,
	})
}
...
func parse(c *casket.Controller) (*Metrics, error) {
	var (
		metrics *Metrics
		err     error
	)
	...
	for c.Next() {
		...
		for c.NextBlock() {
			...
			case "address":
				if metrics.useCasketAddr {
					return nil, c.Err("prometheus: address and use_casket_addr options may not be used together")
				}
				args = c.RemainingArgs()
				if len(args) != 1 {
					return nil, c.ArgErr()
			...
			case "use_casket_addr":
				if addrSet {
					return nil, c.Err("prometheus: address and use_casket_addr options may not be used together")
				}
				metrics.useCasketAddr = true
			case "label":
				args = c.RemainingArgs()
				if len(args) != 2 {
					return nil, c.ArgErr()
