...
var (
	casketLimiter *CasketLimiter
)

func init() {

	casketLimiter = NewCasketLimiter()
}
		...
		for _, res := range rule.Resources {
			...
			*/
			...
			for _, keys := range sliceKeysOnlyWithKey {
				...
				if found := casketLimiter.CheckKeyExists(keysJoined); found {
					ret := casketLimiter.Allow(keys, rule)
					if !ret {
						retryAfter = casketLimiter.RetryAfter(keys)
