...
type CasketLimiter struct {
	Keys map[string]*rate.Limiter
	sync.Mutex
}

func NewCasketLimiter() *CasketLimiter {

	return &CasketLimiter{
		Keys: make(map[string]*rate.Limiter),
	}
}

// Allow is just a shortcut for AllowN
func (cl *CasketLimiter) Allow(keys []string, rule Rule) bool {

	return cl.AllowN(keys, rule, 1)
}

// AllowN check if n count are allowed for a specific key
func (cl *CasketLimiter) AllowN(keys []string, rule Rule, n int) bool {
...
func (cl *CasketLimiter) CheckKeyExists(k string) bool {
...
func (cl *CasketLimiter) RetryAfter(keys []string) time.Duration {
...
func (cl *CasketLimiter) Reserve(keys []string) bool {
